...
* Clone taskcluster monorepo: `git clone https://github.com/taskcluster/taskcluster`
...
### Windows - multiuser build
 ...
 7. Create `C:\generic-worker\generic-worker.config` with appropriate values.
...
### Linux insecure/multiuser build
 ...
 5. Download `generic-worker`, `taskcluster-proxy`, `livelog`, and `start-worker` to `/usr/local/bin`:

    ```
    cd /usr/local/bin
    curl -L "https://github.com/taskcluster/taskcluster/releases/download/v99.0.0/generic-worker-multiuser-linux-amd64" > generic-worker
